local c = require('spaceterminal.colors')
local cfg = vim.g.spaceterminal_config
local util = require("spaceterminal.util")
...
hl.syntax = {
    String = {fg = c.purple, fmt = cfg.code_style.strings},
...
if vim.api.nvim_call_function("has", { "nvim-0.8" }) == 1 then
    hl.treesitter = {
        ...
        ["@attribute.typescript"] = colors.Green,
        ["@boolean"] = colors.Orange,
        ["@character"] = colors.Orange,
        ["@comment"] = {fg = c.grey, fmt = cfg.code_style.comments},
...
hl.plugins.whichkey = {
    ...
    WhichKeySeparator = colors.Purple
}
...
hl.plugins.diffview = {
    ...
    DiffviewFilePanelInsertions = colors.Purple,
    DiffviewFilePanelDeletions = colors.Red,
    DiffviewStatusAdded = colors.Purple,
...
hl.plugins.gitsigns = {
    GitSignsAdd = colors.Purple,
    GitSignsAddLn = colors.Purple,
    GitSignsAddNr = colors.Purple,
...
hl.plugins.ts_rainbow2 = {
    ...
    TSRainbowBlue = colors.Green,
    TSRainbowOrange = colors.Orange,
    TSRainbowGreen = colors.Purple,
...
hl.plugins.rainbow_delimiters = {
    ...
    RainbowDelimiterBlue = colors.Green,
    RainbowDelimiterOrange = colors.Orange,
    RainbowDelimiterGreen = colors.Purple,
...
hl.plugins.indent_blankline = {
    ...
    IndentBlanklineIndent2 = colors.Purple,
...
hl.plugins.mini = {
    ...
    MiniDiffSignAdd = colors.Purple,
    MiniDiffSignChange = colors.Green,
...
hl.langs.markdown = {
    ...
    markdownCode = colors.Purple,
    markdownCodeBlock = colors.Purple,
    ...
    markdownUrlTitleDelimiter = colors.Purple
}
...
hl.langs.php = {
    ...
    phpHereDoc = colors.Purple,
    phpNowDoc = colors.Purple,
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('spaceterminal.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "spaceterminal.nvim" })
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.spaceterminal_config.highlights) do
